...
namespace NetWebScript.Debug.Engine.Debug
{
    ...
    class PendingBreakpoint : IDebugPendingBreakpoint2
    {
        ...
        private readonly IDebugBreakpointRequest2 m_pBPRequest;
        private readonly BP_REQUEST_INFO m_bpRequestInfo;
        private readonly NWSEngine m_engine;
        //private BreakpointManager m_bpManager;

        private readonly List<BoundBreakpoint> m_boundBreakpoints = new List<BoundBreakpoint>();

        private bool m_enabled;
        private bool m_deleted;

        public PendingBreakpoint(IDebugBreakpointRequest2 pBPRequest, NWSEngine engine)
        {
            m_pBPRequest = pBPRequest;
            BP_REQUEST_INFO[] requestInfo = new BP_REQUEST_INFO[1];
            EngineUtils.RequireOk(m_pBPRequest.GetRequestInfo(enum_BPREQI_FIELDS.BPREQI_BPLOCATION, requestInfo));
        ...
        int IDebugPendingBreakpoint2.Bind()
        {
            if (CanBind())
            {
                ...
                if (!hasPoints)
                {
                    Trace.TraceInformation("Breakpoint NOT FOUND : file {0}, line {1}, col {2}", documentName, startRow, startCol);
                    return Constants.S_FALSE;
                }

                Trace.TraceInformation("Breakpoint found : file {0}, line {1}, col {2}", documentName, startRow, startCol);
        ...
        int IDebugPendingBreakpoint2.Enable(int fEnable)
        {
            lock (m_boundBreakpoints)
            {
                ...
                foreach (BoundBreakpoint bp in m_boundBreakpoints)
                {
        ...
        int IDebugPendingBreakpoint2.EnumBoundBreakpoints(out IEnumDebugBoundBreakpoints2 ppEnum)
        {
            lock (m_boundBreakpoints)
            {
                ppEnum = new BoundBreakpoints(m_boundBreakpoints.ToArray());
            }
            return Constants.S_OK;
        }
        ...
        {
            Trace.TraceWarning("int IDebugPendingBreakpoint2.SetCondition(BP_CONDITION bpCondition");
        ...
        {
            Trace.TraceWarning("int IDebugPendingBreakpoint2.SetPassCount(BP_PASSCOUNT bpPassCount)");
            throw new NotImplementedException();
        }
        ...
        internal void OnBoundBreakpointDeleted(BoundBreakpoint boundBreakpoint)
        {
            lock (m_boundBreakpoints)
            {
                m_boundBreakpoints.Remove(boundBreakpoint);
        ...
        private void GetPosition(out string documentName, out int startCol, out int startRow)
        {
            ...
            startCol = (int)startPosition[0].dwColumn + 1;
            startRow = (int)startPosition[0].dwLine + 1;
        }


        internal void OnNewModule(ScriptProgram program, ScriptModule module)
        {
            string documentName;
            int startCol, startRow;
